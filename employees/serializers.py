...
class EmployeeSerializer(serializers.HyperlinkedModelSerializer):
    ...
    adminuser = serializers.PrimaryKeyRelatedField(queryset=AdminUser.objects.none())
    
    class Meta:
        model = Employee
        fields = ('id', 'url', 'branch', 'adminuser', 'employee_id', 'email', 'first_name', 'middle_name', 
                   ...
                   'skills_qualifications', 'next_of_kin_name', 'next_of_kin_relationship', 'next_of_kin_phone_number',
                    'supervised_employees', 'created_at', 'updated_at', 'bank_name', 'account_number',
                   'account_name', 'pension_id', 'tax_id',
                   )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Customize the queryset for adminuser field
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            # Filter the queryset based on the current user's organization
            self.fields['adminuser'].queryset = AdminUser.objects.filter(id=request.user.id)
